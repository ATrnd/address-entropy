...
contract AddressDataEntropySafetyTest is Test {
    ...
    function test_ZeroAddressErrorHandling() public {
        ...
        vm.prank(user1);
        bytes32 entropy = proxy.getEntropy(12_345, user1);
        ...
        verifySafetyFallbackEvent(logs, COMPONENT_ENTROPY_GENERATION, FUNC_GET_ENTROPY, ERROR_ZERO_ADDRESS);
    }
    ...
    function test_ZeroSegmentErrorHandling() public {
        ...
        // Verify safety event
        Vm.Log[] memory logs = vm.getRecordedLogs();
        verifySafetyFallbackEvent(logs, COMPONENT_SEGMENT_EXTRACTION, FUNC_EXTRACT_ADDRESS_SEGMENT, ERROR_ZERO_SEGMENT);
    }
    ...
    function test_OutOfBoundsIndexErrorHandling() public {
        ...
        verifySafetyFallbackEvent(
            logs, COMPONENT_SEGMENT_EXTRACTION, FUNC_EXTRACT_ADDRESS_SEGMENT, ERROR_SEGMENT_INDEX_OUT_OF_BOUNDS
        );
    }
    ...
    function test_CascadingFailureRecovery() public {
        ...
        vm.prank(user1);
        bytes32 entropy = proxy.getEntropy(54_321, user1);
        ...
        bool errorCounterIncremented = proxy.getEntropyGenerationZeroAddressCount() > initialZeroAddressCount
    ...
    function test_ErrorStateRecovery() public {
        ...
        vm.prank(user1);
        bytes32 errorEntropy = proxy.getEntropy(11_111, user1);
        ...
        vm.prank(user1);
        bytes32 normalEntropy = proxy.getEntropy(22_222, user1);
    ...
    function test_ComponentErrorIsolation() public {
        ...
        assertEq(
            proxy.getEntropyGenerationCycleDisruptionCount(),
            0,
            "Entropy generation should not have cycle disruption errors"
        );
        assertEq(
            proxy.getEntropyGenerationZeroSegmentCount(), 0, "Entropy generation should not have zero segment errors"
        );

        // Test specific error counts
        assertEq(
            proxy.getComponentTotalErrorCount(COMPONENT_ADDRESS_EXTRACTION), 1, "Address extraction total should be 1"
        );
        assertEq(
            proxy.getComponentTotalErrorCount(COMPONENT_SEGMENT_EXTRACTION), 0, "Segment extraction total should be 0"
        );
        assertEq(
            proxy.getComponentTotalErrorCount(COMPONENT_ENTROPY_GENERATION), 0, "Entropy generation total should be 0"
        );
        ...
        assertGt(
            proxy.getAddressExtractionZeroAddressCount(), 0, "Address extraction should still have zero address errors"
        );
        assertGt(
            proxy.getSegmentExtractionZeroSegmentCount(), 0, "Segment extraction should now have zero segment errors"
        );
        assertEq(
            proxy.getEntropyGenerationCycleDisruptionCount(),
            0,
            "Entropy generation should still not have cycle disruption errors"
        );
        assertEq(
            proxy.getEntropyGenerationZeroSegmentCount(),
            0,
            "Entropy generation should still not have zero segment errors"
        );
    }
    ...
    function test_ErrorCodeIsolation() public {
        ...
        assertEq(proxy.getSegmentExtractionZeroSegmentCount(), 1, "Zero segment error count should be 1");
        assertEq(proxy.getSegmentExtractionOutOfBoundsCount(), 1, "Out of bounds error count should be 1");
    ...
    function test_ExtremeBoundaryConditions() public {
        ...
        vm.prank(user1);
        bytes32 entropy3 = proxy.getEntropy(12_345, user1);
    ...
    function test_EmergencyEntropyVariation() public {
        ...
        vm.prank(user1);
        bytes32 emergency1 = proxy.getEntropy(55_555, user1);
        vm.prank(user1);
        bytes32 emergency2 = proxy.getEntropy(55_555, user1);
        ...
        proxy.injectZeroAddressAtCurrentIndex();
        vm.prank(user1);
        bytes32 emergency3 = proxy.getEntropy(66_666, user1);
    ...
    function test_ContinuousOperationUnderErrors() public {
        ...
        for (uint256 i = 0; i < 10; i++) {
            vm.prank(user1);
            entropies[i] = proxy.getEntropy(70_000 + i, user1);
    ...
    function test_RecoveryAfterExtendedErrors() public {
        ...
        for (uint256 i = 0; i < 5; i++) {
            vm.prank(user1);
            errorEntropies[i] = proxy.getEntropy(80_000 + i, user1);
        }
        ...
        for (uint256 i = 0; i < 5; i++) {
            vm.prank(user1);
            normalEntropies[i] = proxy.getEntropy(90_000 + i, user1);
        }
        ...
        for (uint256 i = 0; i < 5; i++) {
            ...
            assertTrue(
                errorEntropies[i] != normalEntropies[i], "Error and normal phases should produce different entropy"
            );
        }
    }
    ...
    function test_SegmentIndexBoundaryValues() public {
        ...
        for (uint256 i = 0; i < 4; i++) {
            ...
            assertTrue(
                segment != bytes5(0),
                string(abi.encodePacked("Valid index ", vm.toString(i), " should produce non-zero segment"))
            );
        }
    ...
    function test_EmergencyEntropyDiversity() public {
        ...
        for (uint256 i = 0; i < 10; i++) {
            for (uint256 j = i + 1; j < 10; j++) {
                assertTrue(emergencyEntropies[i] != emergencyEntropies[j], "Emergency entropy should be diverse");
            }
        }
    ...
    function test_RecoveryFromZeroAddress() public {
        ...
        assertTrue(proxy.getEntropyGenerationZeroAddressCount() > initialErrorCount, "Error count should increment");
